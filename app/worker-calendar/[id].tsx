...
export default function WorkerCalendar() {
  ...
  const monthlySalary = parseInt(typeof salary === 'string' ? salary : salary?.[0] || '15000');
  ...
  const fetchWorkerData = async () => {
    try {
      ...
      const applicationsQuery = await getDocs(
        query(collection(db, 'applications'), where('applicantId', '==', workerId))
      );
      
      if (!applicationsQuery.empty) {
        const workerApp = applicationsQuery.docs[0].data();
  ...
  const markAttendance = async (status) => {
    ...
    try {
      ...
      await setDoc(doc(db, 'attendance', attendanceId), {
        employerId: user.uid,
        workerId: workerId,
        workerName: workerData?.applicantName || 'Unknown',
      ...
      Alert.alert(
        'Success', 
        `Marked ${workerData?.applicantName || 'Worker'} as ${status} for ${selectedDate}`
      );
    } catch (error) {
  ...
  const handlePayment = async () => {
    ...
    Alert.alert(
      'Confirm Payment',
      `Pay ₹${monthlyTotal.toLocaleString()} to ${workerData?.applicantName}?`,
      [
  ...
  const processPayment = async () => {
    try {
      ...
      await addDoc(collection(db, 'payments'), {
        employerId: user.uid,
        employerName: 'Organization', // You might want to fetch this
        workerId: workerId,
        workerName: workerData?.applicantName,
        jobTitle: workerJobTitle,
        amount: monthlyTotal,
        workDays: workDays,
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={styles.headerInfo}>
          <Text style={styles.headerTitle}>{workerData?.applicantName || 'Worker'}</Text>
